...
* [Solidity and Fullnode Deployment (English)](./Solidity_and_Full_Node_Deployment_EN.md)

## Running a local node and connecting to the public testnet 

* Use the [Testnet Config](https://github.com/MidasCore/java-mcashchain/blob/master/src/main/resources/config-testnet.conf). 


## Running a Supernode for mainnet

* Use the executable JAR(Recommended way)

```bash
java -jar FullNode.jar -p your private key --witness -c your config.conf(Example：/data/resources/config.conf)
Example:
java -jar FullNode.jar -p 650950B193DDDDB35B6E48912DD28F7AB0E7140C1BFDEFD493348F02295BD812 --witness -c /data/resources/config.conf

```

This is similar to running a private testnet, except that the IPs in the `config.conf` are officially declared by Mcashchain.
...
### Running a Super Node for private testnet
* use master branch
...
```bash
cd build/libs
java -jar FullNode.jar -p your private key --witness -c your config.conf (Example：/data/resource/config.conf)
Example:
java -jar FullNode.jar -p 650950B193DDDDB35B6E48912DD28F7AB0E7140C1BFDEFD493348F02295BD812 --witness -c /data/resource/config.conf
...
Read the [Advanced Configurations](src/main/java/io/midasprotocol/core/config/README.md).
