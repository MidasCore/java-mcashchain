...
public class DepositImpl implements Deposit {
	...
	private HashMap<Key, Value> stakeChangeCache = new HashMap<>();
	private HashMap<Key, Value> stakeAccountCache = new HashMap<>();
	private HashMap<Key, Value> codeCache = new HashMap<>();
	private HashMap<Key, Value> contractCache = new HashMap<>();

	private HashMap<Key, Value> votesChangeCache = new HashMap<>();
	...
	private VoteChangeStore getVotesChangeStore() {
		return dbManager.getVoteChangeStore();
	}
	...
	private StakeChangeStore getStakeChangeStore() {
		return dbManager.getStakeChangeStore();
	}

	private StakeAccountStore getStakeAccountStore() {
		return dbManager.getStakeAccountStore();
	}
	...
	public synchronized VoteChangeCapsule getVoteChangeCapsule(byte[] address) {
		Key key = new Key(address);
		if (votesChangeCache.containsKey(key)) {
			return votesChangeCache.get(key).getVoteChange();
		}

		VoteChangeCapsule voteChangeCapsule;
		if (parent != null) {
			voteChangeCapsule = parent.getVoteChangeCapsule(address);
		} else {
			voteChangeCapsule = getVotesChangeStore().get(address);
		}

		if (voteChangeCapsule != null) {
			votesChangeCache.put(key, Value.create(voteChangeCapsule.getData()));
		}
...
//			}
	...
	public void putVotes(Key key, Value value) {
		votesChangeCache.put(key, value);
	}
	...
	public void putStakeChange(Key key, Value value) {
		stakeChangeCache.put(key, value);
	}

	@Override
	public void putStakeAccount(Key key, Value value) {
		stakeAccountCache.put(key, value);
	}
	...
	@Override
	public synchronized StakeChangeCapsule getStakeChangeCapsule(byte[] address) {
		Key key = Key.create(address);
		if (stakeChangeCache.containsKey(key)) {
			return stakeChangeCache.get(key).getStakeChange();
		}

		StakeChangeCapsule stakeChangeCapsule;
		if (parent != null) {
			stakeChangeCapsule = parent.getStakeChangeCapsule(address);
		} else {
			stakeChangeCapsule = getStakeChangeStore().get(address);
		}

		if (stakeChangeCapsule != null) {
			stakeChangeCache.put(key, Value.create(stakeChangeCapsule.getData()));
		}
		return stakeChangeCapsule;
	}

	@Override
	public synchronized StakeAccountCapsule getStakeAccountCapsule(byte[] address) {
		Key key = Key.create(address);
		if (stakeAccountCache.containsKey(key)) {
			return stakeAccountCache.get(key).getStakeAccount();
		}

		StakeAccountCapsule stakeAccountCapsule;
		if (parent != null) {
			stakeAccountCapsule = parent.getStakeAccountCapsule(address);
		} else {
			stakeAccountCapsule = getStakeAccountStore().get(address);
		}
		if (stakeAccountCapsule != null) {
			stakeAccountCache.put(key, Value.create(stakeAccountCapsule.getData()));
		}
		return stakeAccountCapsule;
	}
	...
	private void commitVoteCache(Deposit deposit) {
		votesChangeCache.forEach(((key, value) -> {
			if (value.getType().isDirty() || value.getType().isCreate()) {
				if (deposit != null) {
					deposit.putVotes(key, value);
				} else {
					getVotesChangeStore().put(key.getData(), value.getVoteChange());
				}
			}
		}));
	}
	...
	private void commitStakeChangeCache(Deposit deposit) {
		stakeChangeCache.forEach((key, value) -> {
			if (value.getType().isCreate() || value.getType().isDirty()) {
				if (deposit != null) {
					deposit.putStakeChange(key, value);
				} else {
					getStakeChangeStore().put(key.getData(), value.getStakeChange());
				}
			}
		});
	}

	private void commitStakeAccountCache(Deposit deposit) {
		stakeAccountCache.forEach((key, value) -> {
			if (value.getType().isCreate() || value.getType().isDirty()) {
				if (deposit != null) {
					deposit.putStakeAccount(key, value);
				} else {
					getStakeAccountStore().put(key.getData(), value.getStakeAccount());
				}
			}
		});
	}
	...
	public void putVoteChangeValue(byte[] address, VoteChangeCapsule voteChangeCapsule) {
		Key key = new Key(address);
		votesChangeCache.put(key, new Value(voteChangeCapsule.getData(), Type.VALUE_TYPE_CREATE));
	}
	...
	public void putStakeChangeValue(byte[] address, StakeChangeCapsule stakeChangeCapsule) {
		Key key = new Key(address);
		stakeChangeCache.put(key, new Value(stakeChangeCapsule.getData(), Type.VALUE_TYPE_CREATE));
	}

	@Override
	public void putStakeAccountValue(byte[] address, StakeAccountCapsule stakeAccountCapsule) {
		Key key = new Key(address);
		stakeAccountCache.put(key, new Value(stakeAccountCapsule.getData(), Type.VALUE_TYPE_CREATE));
	}
	...
	public synchronized void commit() {
		...
		commitStakeChangeCache(deposit);
